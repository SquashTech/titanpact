...
| Scallywag | Iron (unlock) | **Broadside** | On the bench, loads a cannonball a round (up to `BROADSIDE_MAGAZINE` = 4); on entering, fires them all — `BROADSIDE_SHOT` = 5% of max HP to both enemies per ball, direct, the magazine spent. Two ids under one name (the load, the firing); the ONLY reaction that fires from the bench (`whileBenched`). | new · `whileBenched`, `maxMagnitude`, `perHeldStatus` | Plunder (Corsair) |
