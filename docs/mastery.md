...
| 4 | **Author 35 signatures.** Parallelisable from 3; ships hero by hero (an unauthored hero's tenth pip pays nothing, which is what today pays). | Every hero has one, on the template — a hit or a verb at the hero's primary, Late-priced, never a bare nuke. | **DRAFTED 2026-09-14, under review.** All 36 point at a signature (`src/data/signatures.ts`): a hit sized like the type's Late moves plus the hero's own verb — the thing its paths keep circling — no two alike. Two written to the tests' pinned decisions rather than the first draft: Roost Guard grants Defense only (Wisdom off-Mind is a decision the Iron test guards), and Crag's is *Groundsplit* (the slate already had a Fault Line). Every per-type slate test now filters the catalog out, since a signature is a hero's, not a type's. |
