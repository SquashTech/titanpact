 ...
 *                  from off the near edge and bursting, one <i> a ball.
...
/** Balls in a broadside. The volley reads as a volley; the damage popup carries the real count. */
const CANNONBALL_PARTICLES = 3;
...
export function TypeFx({ type, kind }: { type: string; kind: 'element' | 'buff' | 'cannonball' }) {
  ...
  const count = kind === 'cannonball' ? CANNONBALL_PARTICLES : buff ? BUFF_PARTICLES : (PARTICLES[type] ?? 5);
